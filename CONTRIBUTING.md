...
Pull Request 合入主仓库的 `develop` 分支。只有任务明确要求 `keep-shitting`
时，才允许把 PR 目标分支设为 `keep-shitting`。

## 基本原则

- 主仓库日常集成分支是 `develop`。
- 禁止向 `main` 发起日常开发 PR。
- `keep-shitting` 只用于明确指定该目标分支的特殊任务。
- 禁止直接向 `develop` 或 `main` push 功能、修复或文档修改。
- 必须从主仓库最新目标分支创建个人 fork 中的独立分支。
- PR 必须指向主仓库允许的目标分支，默认是 `develop`。
...
AI agent 在运行 `git commit` 前必须阅读 [docs/git-workflow.md](docs/git-workflow.md)
和 [.agents/git-commit-checklist.md](.agents/git-commit-checklist.md)。
...
3. 拉取主仓库最新目标分支，默认是 `develop`。
4. 从 `upstream/develop` 创建个人工作分支；
5. 在个人分支提交修改。
6. 推送到个人 fork。
7. 使用 `gh pr create` 向主仓库允许的目标分支发起 PR，默认是 `develop`。
...
- 当前分支来自最新 `upstream/develop`，或任务明确要求时来自最新
  `upstream/keep-shitting`。
- PR base 是 `develop` 或任务明确要求的 `keep-shitting`，不是 `main`。
...
- 不合并目标分支不是 `develop` 或明确指定 `keep-shitting` 的日常开发 PR。
- 不合并来自主仓库分支的成员 PR。
