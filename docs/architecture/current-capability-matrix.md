...
| Knowledge 知识库 CRUD、上传和 ingestion 写入链路 | 部分实现 | Knowledge PostgreSQL repository、知识库 CRUD、文档上传、File handoff、asynq ingestion 入队、Parser Service client、chunker、embedding、chunk 持久化、Qdrant/in-memory vector index 写入、document lifecycle、chunks/content、`knowledge-queries` 检索，以及 env-gated Gateway -> Knowledge -> QA RAG 最小 smoke。 | Gateway Knowledge active paths；Knowledge service OpenAPI。 | 真实 AI Gateway embedding/rerank provider、真实 Qdrant query 运行证据、MCP/#125 完整 E2E 和并发/外部副作用一致性仍需补齐。 | #152、#200、#226、#304 |
| Parser Runtime | 部分实现 | `services/parser/` Python/FastAPI runtime、`docs/services/parser/api/internal.openapi.yaml`、`services/parser/api/openapi.yaml` 实现本地副本、service-token auth、PaddleOCR optional extra 和 Knowledge Parser service client。 | Parser 内部 `/internal/v1/parsed-documents` API；不通过 gateway 暴露。 | 真实 PaddleOCR 模型 smoke、部署资源配置和 Knowledge/File/Redis 全链路 smoke 仍需补齐。 | #83、#226 |
| QA 会话、消息、配置、引用和统计资源 | 部分实现 | `services/qa/` 会话/消息 API、SSE 事件、config versions、citations、retrieval test/metrics 资源、AI Gateway chat client、B-03 非流式 ResponseRun / Agent Loop MVP（本分支），以及 env-gated Gateway RAG smoke 的 answer/citation 验收。 | Gateway QA active paths；QA service OpenAPI。 | 真实 provider 运行证据、MCP/前端完整 E2E、citation snapshot/detail/batch query 和权限一致性仍需收口。 | #157、#89、#217、#219、#210、#304 |
...
| 本地联调环境 | 部分实现 | 根 `deploy/docker-compose.yml` 只提供 PostgreSQL、Redis、Qdrant、MinIO 和 `minio-init`；migration、seed 和业务服务均按 host-run 文档执行；Issue #304 已提供 env-gated 最小 RAG smoke runbook。 | 本地运行手册见 `deploy/README.md` 和 `docs/runbooks/local-integration.md`。 | 现有 seed data 覆盖本地登录、基础报告类型、示例知识库和 AI profile placeholder；真实 provider/Qdrant 运行证据、统一跨服务 smoke 和一键 E2E 验收脚本仍缺。 | #125、#150、#304 |
...
1. 本地联调要继续明确根级 Compose 只负责基础设施，业务服务必须 host-run；生产/准生产部署流水线和 #125 完整 smoke 仍需后续收口。
