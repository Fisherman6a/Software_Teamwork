...
前端运行时展示的版本号以 `apps/web/package.json` 的 `version` 字段为唯一来源。Vite 在构建时读取该字段并注入只读常量，页面组件不得再手写第二份版本号。

认证后顶栏的版本号可点击联网检查 `upstream/develop` 最新提交。检查结果只用于提示当前构建是否落后远端最新提交、落后多少个 commits，并给出终端更新命令；前端页面不得自动执行 Git 更新、改写本地文件或提升版本号。

涉及前端大更新或用户可见能力变化的 PR，需要同步提升 `apps/web/package.json` 的 `version`，并在 PR 描述中说明版本变化；只改文档、测试或纯内部重构时可按维护者判断不提升版本。
