...
检索、错误 envelope 和 request id。A-021 已新增 env-gated ingestion 真实依赖
smoke，覆盖一个 fixture 文档从 File Service、Knowledge worker 到
...
knowledge service
   ...
   +--> RAGFlow runtime API/worker for parse, chunks, embedding, index, retrieval
```
...
| `dependency_error` | `502` | PostgreSQL、Redis、MinIO、RAGFlow runtime、AI Gateway 或其他依赖失败。 |
