...
> 的 Knowledge MCP Server、QA discovery 和默认工具白名单变更。#505 合并前，
> `develop` 默认只提供内置 `search_knowledge`，模型看不到本文四个 `knowledge__*`
> 工具。#505 合并后创建的新默认 Agent 配置会包含四个名称；已有或自定义的
> `enabledToolNames` 不会被静默改写，必须显式加入这四个名称，否则
> `Policy.FilterTools` 仍会将它们过滤掉。
...
- QA 默认 alias 后的工具名必须与本文一致；#505 合并前不得把本文误读为当前
  `develop` 已启用这些工具。
