...
| Parser host-run 前置条件 | 本地联调通过宿主机 `uv run parser-service` 启动 Parser | Parser 未在 `localhost:8087` 监听或 token 不一致时，Knowledge/Gateway smoke 会失败 | 容易把 Parser 未启动误判为 Knowledge 代码问题 | runbook 已记录 infra Docker + host-run Parser/File/Knowledge 的启动顺序。 |
