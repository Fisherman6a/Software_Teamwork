...
- 生成请求可通过 `options.knowledgeBaseIds` 等检索参数在配置了 Knowledge 服务时获取受控材料上下文；`document` 不直接访问 Knowledge runtime/doc engine 或 Qdrant。
...
`document` 不负责用户登录、会话签发、RBAC 源数据、知识库索引、Knowledge runtime/doc-engine 检索实现、provider API key 存储、MinIO object key 暴露、QA 聊天流程或前端页面。
...
报告生成需要材料上下文时，应通过报告素材业务资源和 knowledge 检索能力获取。`document` 不直接访问 Knowledge runtime/doc engine 或 Qdrant，不保存知识库切片索引状态，也不绕过 `knowledge` 读取受控知识库数据。
...
工具层不得绕过 Document service 边界直连数据库、MinIO、Knowledge runtime/doc engine、Qdrant 或 provider。工具响应只返回安全摘要、业务资源 ID、进度和错误码，不返回 prompt、provider 原始错误、内部 URL、对象存储路径、File Service 内部引用或完整工具私有参数。`export_report_docx` 只走当前基础 DOCX 导出路径，不代表 Pandoc/LibreOffice 富 DOCX 已可用。
