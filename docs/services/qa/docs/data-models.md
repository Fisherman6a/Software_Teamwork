...
| `system_prompt` | text | 全局 Agent 系统提示词。与检索参数、Agent 策略和工具白名单一起原子版本化。创建新版本时如显式提供则必须非空且 ≤ 20000 bytes；如省略则继承当前 active 版本的提示词。无 active 版本且未提供时回退到 `AGENT_SYSTEM_PROMPT` bootstrap 值。回答通过 `qaConfigVersionId` 追溯生成时使用的提示词版本。管理员可通过 `qa:settings:read` 读取完整内容；普通用户、QA 响应、SSE、错误、日志和指标不得返回完整提示词。 |
