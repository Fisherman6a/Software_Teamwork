...
| `system_prompt` | text | 全局 Agent 系统提示词。非空，去除首尾空白后 `octet_length` ≤ 20000。仅管理员（`qa:settings:read`/`qa:settings:write`）可读写完整内容。 |
...
- `octet_length(system_prompt) BETWEEN 1 AND 20000`。
