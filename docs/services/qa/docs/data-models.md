...
| `system_prompt` | text | 全局 Agent 系统提示词。与检索参数、Agent 策略和工具白名单一起原子版本化。非空，最大 20000 bytes。回答通过 `qaConfigVersionId` 追溯生成时使用的提示词版本。管理员可通过 `qa:settings:read` 读取完整内容；普通用户、QA 响应、SSE、错误、日志和指标不得返回完整提示词。 |
