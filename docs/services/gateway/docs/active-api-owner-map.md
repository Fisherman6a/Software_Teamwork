...
- Active means the public method/path/schema is part of the collaboration contract.
  Check `docs/services/gateway/docs/implementation.md` and smoke records before
  treating an active path as end-to-end ready with real owner-service
  dependencies.
...
| `knowledge` | 20 | Knowledge bases, knowledge documents, chunks, retrieval, and parser runtime config. |
| `ai-gateway` | 5 | Model profile runtime configuration exposed through gateway admin paths. |
| `document` | 45 | Report templates, materials, records, outlines, sections, jobs, events, files, settings, statistics, and logs. |
...
| `GET` | `/api/v1/app-version/freshness` | `gateway` | `app-version` | `getAppVersionFreshness` | `none` |
...
| `POST` | `/api/v1/knowledge-bases/{knowledgeBaseId}/document-deletion-jobs` | `knowledge` | `documents` | `createKnowledgeBaseDocumentDeletionJob` | `bearerAuth` |
...
| `GET` | `/api/v1/reports/{reportId}/events/stream` | `document` | `report-generation` | `streamReportEvents` | `bearerAuth` |
