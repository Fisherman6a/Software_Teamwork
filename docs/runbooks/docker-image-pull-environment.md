...
Python 依赖和 artifact 只按 `./scripts/local/check.sh` / `--china` 输出的手工命令执行。
不要把 `pyproject.toml` 或 `uv.lock` 的默认 URL 改成第三方代理。
Go 后端二进制构建期间的模块下载由 `config/base.yaml` 里的 `GOPROXY` / `GOSUMDB`
控制；默认是官方 `proxy.golang.org` / `sum.golang.org`。中国大陆网络用
`./scripts/local/check.sh --china` 查看 Go mirror 建议，也不属于 Docker
registry 问题。`./scripts/local/start.sh` 不执行 Go/uv 下载，也不主动拉 Docker 镜像。

## 源策略契约

当前契约是默认官方源、国内网络显式 `--china`。
PR review 和 agent 检查应把 active 第三方镜像默认值视为回归；缺少这些 active 默认值
不是回归。中国大陆用户的一等路径是运行 `./scripts/local/check.sh --china` 查看手工命令，
或使用本机未提交的 `.env.local` / 企业镜像覆盖；启动命令只消费已准备好的镜像和依赖。
...
该模式只打印 `POSTGRES_IMAGE`、`REDIS_IMAGE`、`MINIO_IMAGE`、
`MINIO_MC_IMAGE` 和 `KNOWLEDGE_RUNTIME_ELASTICSEARCH_IMAGE`
的 `docker.1ms.run` registry rewrite 建议，不改写 `.env.local`。

手工 `docker pull` 只在用户明确执行时发生。`start.sh` 后续使用
`docker compose up --pull never` 启动已经存在的镜像，再执行 migration 和 seed。只想验证
Docker 配置时：
...
- `config/base.yaml` 和 `.env.example` 不能默认启用第三方基础设施镜像 rewrite；大陆网络使用
  `./scripts/local/check.sh --china` 或本地 `.env.local` override。
