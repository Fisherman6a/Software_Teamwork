...
| 本地启动脚本 / 文档 | `bash -n scripts/local/check.sh scripts/local/start.sh scripts/local/stop.sh scripts/local/clean.sh scripts/local/lib/*.sh scripts/config/load-profile.sh`；`python3 scripts/verify_local_seed_contract.py`；确认 README/deploy/runbook 第一屏仍是 `cp .env.example .env.local`、`./scripts/local/check.sh`、`./scripts/local/start.sh`、`cd apps/web && bun install && bun run dev`，默认源保持官方，`check.sh` 只检查并打印官方/中国大陆手工下载建议，不下载、不构建、不 pull 镜像、不执行 `uv sync`，`--china` 不改写 `config/` 或 `.env.local`，`start.sh` 不执行 `go mod download`、`go run module@version`、`uv sync`、runtime artifact 下载或 Docker pull，本地入口脚本都有开始/成功/失败命令行摘要，`start.sh` 在 migration/seed 前等待 infra health 并汇总早退服务日志，Go module proxy 排障说明区分 `GOPROXY`、Docker registry rewrite、GitHub release/raw 镜像和 `UV_DEFAULT_INDEX`，AI Gateway 本地 seed 使用 `localhost:11434`，stop 脚本按进程组停止 host-run 服务。 |
