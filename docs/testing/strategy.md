...
触发范围以改动影响面为准：改文档只需要文档一致性和 `git diff --check`；改前端页面或 API client 需要前端 check/build/unit，触碰关键流程时再加 E2E smoke；改 Gateway OpenAPI、owner map 或 route 时需要契约校验和前端类型生成检查；改后端 service、repository、SQL 或 migration 时需要对应服务测试，并按风险补充 repository 或 migration apply；改 Docker infra Compose、cloud Docker app stack、基础设施镜像源、runtime dependency 或 CI 配置时，需要同步 runbook/技术决策文档并运行对应 policy/config 检查。跨契约、跨服务或共享能力变更要扩大验证范围，不能只跑离改动最近的一层。
...
| `docker-deploy-checks.yml` | `deploy/docker-compose.yml`、`deploy/docker-compose.cloud.yml`、`deploy/docker/full/**`、Docker runbook/spec、Docker policy/environment scripts | 执行 `python3 scripts/check_docker_policy.py`、Docker policy/environment 单元测试、`check_docker_environment.py --skip-network --clean-env`，并对根级 infra-only Compose 和 cloud Docker app stack 执行 `docker compose ... config --quiet`；cloud Dockerfile/support-file 变更也触发 policy 和 cloud compose config；cloud 模板默认 seed disabled，不执行生产部署。 |
...
| Docker policy | `python3 scripts/check_docker_policy.py`；验证根级 Compose 默认服务只包含 `postgres`、`redis`、`minio`、`minio-init`、`elasticsearch`；根 Compose 不包含 `build:`，cloud Compose 是唯一允许业务 `build:` 的 app stack；基础设施和 cloud 基础镜像默认值不使用 `latest`，`.env.example` 不启用第三方镜像源。 |
| 本地启动脚本 / 文档 | `bash -n scripts/local/start.sh scripts/local/stop.sh scripts/local/clean.sh scripts/local/lib/*.sh scripts/config/load-profile.sh scripts/docker/start.sh scripts/docker/stop.sh scripts/docker/clean.sh`；`sh -n deploy/docker/full/*.sh`；`python3 scripts/verify_local_seed_contract.py`；确认 README/deploy/runbook 第一屏仍覆盖 host-run `cp .env.example .env.local`、`./scripts/local/start.sh`、`cd apps/web && bun install && bun run dev`，并覆盖 cloud Docker `cp deploy/docker/cloud.env.example .env.docker.cloud`、`./scripts/docker/start.sh`；默认源保持官方，`start.sh` 先做宿主机 preflight，并只检查 `.env.local` 已存在，不创建或改写它；之后按需准备 local Go tools、`goose@v3.27.0`、服务二进制、Docker infra images、Knowledge runtime `.venv` 和 artifact；cloud Docker path 校验 `.env.docker.cloud` 占位值后运行 `up -d --build`，并外接云端 DB/Redis/object storage/runtime/OCR/provider；cloud seed 关闭时不从 `AI_GATEWAY_LOCAL_CHAT_MODEL` 回填 QA/Document 运行时模型。 |
...
| Compose | `CONFIG_SECRET_FILE=.env.example ./scripts/config/load-profile.sh --print-compose-env` 后执行 `docker compose -f deploy/docker-compose.yml --env-file .local/config/dev.env config --quiet`；默认服务清单只能是 `postgres`、`redis`、`minio`、`minio-init`、`elasticsearch`。Cloud path 还要执行 `docker compose -f deploy/docker-compose.cloud.yml --env-file deploy/docker/cloud.env.example config --quiet`。 |
