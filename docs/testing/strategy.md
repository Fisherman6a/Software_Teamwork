...
| 本地启动脚本 / 文档 | `bash -n scripts/local/dev-up.sh scripts/local/run-backend.sh scripts/local/stop-backend.sh`；`python3 scripts/verify_local_seed_contract.py`；确认 README/deploy/runbook 第一屏仍是 `cp deploy/.env.example deploy/.env`、`./scripts/local/dev-up.sh`、`./scripts/local/run-backend.sh`、`cd apps/web && bun install && bun run dev`，并确认本地入口脚本都有开始/成功/失败命令行摘要，`dev-up.sh` 在 migration/seed 前等待 infra health、仅在 `QDRANT_URL` 明确配置时处理 legacy/test-only Qdrant collection、检查 Go module 配置，`run-backend.sh` 预检 Go module 下载并汇总早退服务日志，Go module proxy 排障说明区分 `GOPROXY`、Docker registry rewrite 和 `UV_DEFAULT_INDEX`，AI Gateway 本地 seed 使用 `localhost:11434`，stop 脚本按进程组停止 host-run 服务。 |
