...
云端依赖 Docker 启动只需要 Docker/Compose v2 和一个未跟踪的 `.env.docker.cloud`。这条路径不会启动本地 PostgreSQL、Redis、MinIO、Elasticsearch、Knowledge runtime worker 或本地 OCR；数据库、OCR parser 配置和模型 provider 默认视为已在云端预置，`DOCKER_SEED_ENABLED=false` 是模板默认值。
托管 Redis 需要 ACL/TLS 时在 `.env.docker.cloud` 配置 `GATEWAY_REDIS_USERNAME` / `GATEWAY_REDIS_TLS_ENABLED` 和对应 Document Redis 变量；只有临时云库明确需要写入本地 demo seed 时，才把 `DOCKER_SEED_ENABLED=true`，并替换所有 `<...>`、`local-dev-*`、`local-demo-*`、`change-me` 占位 secret。

第一次使用云端依赖 Docker 路径：

```bash
cp deploy/docker/cloud.env.example .env.docker.cloud
# 填写云端 PostgreSQL、Redis、对象存储、Knowledge runtime 和服务 token；seed 默认关闭
...
`deploy/docker-compose.cloud.yml` 是明确批准的第二条 Docker app stack；它可以 `build:` 业务镜像，但不替代根级本地 infra Compose，也不是生产部署基线。真实云端资源和 secret 由用户或外部平台提供，仓库只提交占位模板。
...
当前 `deploy/docker-compose.yml` 是本地/演示基础设施基线，不是生产部署基线，也不包含业务服务容器。`deploy/docker-compose.cloud.yml` 是云端依赖的一行 Docker 构建/启动链路，默认外接云端 OCR/runtime/model/DB/Redis/object storage，且默认不写入本地 demo seed。
