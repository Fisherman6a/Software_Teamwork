...
- For Docker or Compose changes, check `docs/runbooks/docker-image-pull-environment.md`, `docs/testing/strategy.md`, `.trellis/spec/cicd.md`, `deploy/.env.example`, `scripts/check_docker_policy.py`, and `scripts/check_docker_environment.py`. Flag regressions that add business services to Docker, introduce `build:`, introduce `latest`, drop pinned infra Compose defaults/override variables, break the mainland China registry-rewrite defaults, or make proxy/daemon-mirror workarounds the only documented path.
