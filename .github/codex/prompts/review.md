...
- For Docker or Compose changes, check `docs/runbooks/docker-image-pull-environment.md`, `docs/testing/strategy.md`, `.trellis/spec/cicd.md`, `deploy/.env.example`, `scripts/check_docker_policy.py`, and `scripts/check_docker_environment.py`. Current source policy is official-by-default with explicit `--china` mirror mode. Flag regressions that add business services to Docker, introduce `build:`, introduce `latest`, drop pinned infra Compose defaults/override variables, reintroduce active third-party mirror defaults in `deploy/.env.example`, remove or break the explicit mainland China `--china` registry rewrite path, or make proxy/daemon-mirror workarounds the only documented path.
