...
这条路径用于降低本机压力：本机只负责 Docker 构建和运行业务服务/前端容器，重型依赖走云端或外部服务。它不启动本地 PostgreSQL、Redis、MinIO、Elasticsearch、Knowledge runtime API/worker 或本地 OCR。Knowledge adapter 连接 `VENDOR_RUNTIME_URL`。Cloud seed 默认关闭，数据库、parser config、model profiles、用户和 QA 设置默认视为已由云端预置。
...
- 云端 PostgreSQL：六个服务库 URL 始终需要；只有显式设置
  `DOCKER_SEED_ENABLED=true` 时才需要 `POSTGRES_ADMIN_URL`。
...
- 云端 OCR：只有 `DOCKER_SEED_ENABLED=true` 时才需要 `PADDLEOCR_ACCESS_TOKEN`。
- 云端模型 provider：只有 `DOCKER_SEED_ENABLED=true` 且
  `AI_GATEWAY_LOCAL_SEED_ENABLED` 不为 false 时，才需要
...
默认浏览器入口是 `http://localhost:18080`。脚本会先校验 `.env.docker.cloud` 是否存在并检查关键占位值/本地 demo secret，再执行：
...
- 校验 `.env.docker.cloud`，渲染 `deploy/docker-compose.cloud.yml`，执行 `up -d --build`，
  ...
  seed 默认关闭，显式启用 seed 时会拒绝本地 demo secret。
