...
重复启动或只想拉起 infra 时可加 `--skip-knowledge-runtime-deps`，之后需要时再手工补跑
runtime 下载脚本。
...
`./scripts/local/run-knowledge-parse-stack.sh` 一次性启动 full parse stack。只运行
API-only runtime + Go 后端时，上传只会通过 `/documents/parse` 入队；worker
必须由本地 helper、systemd、K8s/KEDA、supervisor 或其他外部生命周期工具启动。
...
`CONFIG_PROFILE=staging` 选择其它 profile。默认使用官方源。Go modules 下载默认读取
profile 渲染结果里的 `GOPROXY` / `GOSUMDB`；`dev-up.sh` 执行 goose migration、
...
embedding、索引和检索通过 `services/knowledge-runtime` 完成。`./scripts/local/dev-up.sh`
默认会准备 runtime 依赖并下载 NLTK/HuggingFace/GitHub release/raw artifacts；中国大陆网络
加 `--china` 时使用镜像源。如果此前用
...
默认命令会把 runtime 依赖同步到 `.venv` 并使用官方源下载 artifacts；加 `--china`
时会用临时 overlay 把 runtime 依赖同步到 `.venv`，并把 GitHub release、PyPI、
NLTK、HuggingFace、Tika 和 Chrome 下载切到镜像。提交的 `pyproject.toml` / `uv.lock`
仍保持官方 URL。需要企业源时，显式设置本机环境变量或本机 `.env.local`。
...
- 先检查同一宿主机环境中的 Docker、Go、`psql`、`uv`（默认 runtime 准备需要；跳过该步骤时不需要），缺失时直接
  在命令行报错，避免跑到 migration/seed 中途才失败。
- 默认执行 Knowledge runtime 依赖和 artifact 下载；传入 `--china` 时使用镜像源。可用
  `--skip-knowledge-runtime-deps` 或 `LOCAL_SKIP_KNOWLEDGE_RUNTIME_DEPS=1` 跳过该准备步骤。
...
- 如果希望 AI Gateway 的本地默认 profile 也自动指向同一组 provider，在
  ...
  host-run env 可将 `MODEL_ID` / `DOCUMENT_AI_GATEWAY_MODEL` 留空，由 AI Gateway
  根据 `default-chat` profile 决定 provider model。
...
- runtime worker 第一次启动会按需下载 `InfiniFlow/deepdoc` 等 deepdoc 模型。中国大陆
  ...
  `HF_ENDPOINT=https://hf-mirror.com` 的本次进程覆盖；需要只启动 worker 时，可由外部
  生命周期工具或开发者显式运行 `./scripts/local/start-knowledge-runtime-worker.sh`。
...
`.env.local` 设置 `AI_GATEWAY_LOCAL_SEED_ENABLED=true`，`dev-up.sh` 会用
