...
源选择采用官方默认源，国内网络用 `check.sh --china` 查看镜像建议。默认文件不提交
...
`sum.golang.org`。启动脚本不自动下载 Go modules、uv 环境、runtime artifacts，
也不主动 pull Docker images。

第一次启动前先检查本机。检查脚本只检查并输出官方/中国大陆下载建议，不会下载、构建、
pull 镜像、同步 uv 环境或改写 `.env.local`：
...
索引和检索链路还需要在本地 `.env.local` 中填写 provider，并提前按 `check.sh` 输出的
建议准备 runtime `.venv`、NLTK data、Tika jar、cl100k encoding 和 deepdoc 模型。
...
中国大陆网络下，runtime 启动时需要 HuggingFace mirror 才加 `--china`：

```bash
./scripts/local/start.sh --china
```

注意：`start.sh` 只启动已经准备好的 `.venv`，不会执行 `uv sync` 或下载模型/artifact。
...
你手工执行 `check.sh` 输出的 `go build` / `go install` 命令时发生，并读取环境里的
`GOPROXY` / `GOSUMDB`。官方默认值是
`GOPROXY=https://proxy.golang.org,direct` 和 `GOSUMDB=sum.golang.org`。

中国大陆网络使用大陆镜像时，不要把仓库默认文件改成第三方代理；先运行
`./scripts/local/check.sh --china` 查看手工命令建议。
...
`./scripts/local/check.sh --china` 查看 `docker.1ms.run` registry rewrite 建议；企业
...
embedding、索引和检索通过 `services/knowledge-runtime` 完成。启动脚本不会执行
`uv sync` 或 artifact 下载；需要时手工执行：
...
`Get "https://proxy.golang.org/...": i/o timeout`，先运行 `./scripts/local/check.sh --china`
查看 `GOPROXY=https://goproxy.cn,direct` 和 `GOSUMDB=sum.golang.google.cn` 建议。
...
- 检查 Docker、Go、uv、Bun、`psql`、`curl`、Python、`.env.local`、本地工具、服务二进制、
  Compose 配置和 runtime 文件；Docker image 只打印手工拉取建议，不探测本地镜像状态。
- 不下载、不构建、不 pull 镜像、不执行 `uv sync`、不改写 `.env.local`。
- 输出官方源手工命令；`--china` 输出中国大陆镜像建议。

`./scripts/local/start.sh`：

- 不运行 `go mod download`、`go run module@version`、`uv sync`、runtime artifact 下载或
  Docker pull。
- 使用 `.local/tools/config-ctl` 渲染配置，使用 `.local/tools/goose` 执行 migration。
- 使用已有 infra images 启动 Compose，默认 `--pull never`。
